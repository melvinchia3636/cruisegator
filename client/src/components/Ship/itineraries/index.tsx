...
function ConnectedItinerariess({ itineraries_data, id, setLoaded }: ItinerariesProps) {
  const getData = (itiID: number | undefined): void => {
    setLoaded(false);
    axios
      .get(`https://api.cruisegator.thecodeblog.net/ship/itineraries/${itiID}`)
      .then((res) => {
  ...
  return (
    <div className="p-10 mb-16 sm:px-32 overflow-hidden flex flex-col">
      <div className="px-8 py-2 flex items-center">
        ...
        <button type="button" className="w-11" />
      </div>
      {data.map((e, index) => (
        <div
          key={e.id}
          className="w-full bg-white my-1 rounded-lg overflow-hidden"
          style={{ boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}
        >
          <div className="px-8 py-6 flex items-center">
            <h2 className="font-medium w-8/12 text-blue-800 text-xl">
              {toTitleCase(e.title.toLowerCase())}
            </h2>
            <p className="w-3/12 text-gray-700 font-medium text-lg">
              {new Date(e.sailings[0]).toLocaleString('en-US', {
                day: '2-digit',
                month: 'short',
                year: 'numeric',
              })}
            ...
            <button
              type="button"
              className={`flex justify-center mb-5 md:mb-0 transform transition-all duration-500 ${
                e.state ? 'rotate-180' : ''
              }`}
              onClick={() =>
                e.setState(e.state ? 0 : document.querySelector(`#im-${index}`)?.clientHeight)
              }
            >
          ...
          <div className="overflow-hidden transition-all duration-300" style={{ height: e.state }}>
            <div className="p-8 md:p-12" id={`im-${index}`}>
              <img
                alt=""
                className="rounded-lg mt-6 md:hidden"
                src={`https://img.cruisecritic.net/img-cc/map/${e.id}/itinerary_540x405_.png`}
              />
              <div className="flex justify-between">
                <div className="mt-5 itc">
                  {e.itinerary.map((i, innerIndex) => (
                    <div
                      key={i.day}
                      className="grid items-center gap-x-3 gap-y-4 py-1 grid-cols-[16px,20%,80%]"
                    >
                      <style
                        dangerouslySetInnerHTML={{
                          __html: `#it-${index}:before { height: ${
                            (e.itinerary.length - 1) * 36
                          }px }`,
                        }}
                      />
                      <span
                        className={`it relative w-4 h-4 rounded-full ${
                          [1, e.itinerary.length].includes(innerIndex + 1) || i.port !== 'Cruising'
                            ? 'bg-blue-800'
                            : 'border-2 border-blue-800 bg-white'
                        }`}
                        id={`it-${index}`}
                      />
                ...
                <img
                  alt=""
                  className="rounded-lg object-scale-down hidden md:!block"
                  src={`https://img.cruisecritic.net/img-cc/map/${e.id}/itinerary_540x405_.png`}
                />
              </div>
            </div>

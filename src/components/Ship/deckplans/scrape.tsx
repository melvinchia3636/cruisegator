...
const getData  = async (id: string): Promise<void> => {
	...
	const first_html: Document = shipraw_data[2];
	const {0: firstdeck, length: l, [l-1]: lastdeck}: number[] = Array.from(first_html.querySelector(".panel-body")?.querySelectorAll("a") || []).map(e => parseInt((e.href.match(/&deck=(\d+)$/) || [])[1])).filter(e => e);
	...
	console.log(staterooms_inner_raw_group);
